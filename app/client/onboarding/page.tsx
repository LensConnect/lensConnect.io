...
const Page = () => {
  ...
  const fetchPortfolios = async () => {
    ...
    const photographerIds = [...new Set(portfolioData.map((p) => p.photographer_id))]
    ...
    const merged = portfolioData.map((p) => ({
      ...p,
      photographer_profiles: profileData?.find((profile) => profile.id === p.photographer_id),
    }))
        ...
        <div className='mt-12'>
          {!session ? (
            <div className='text-center text-gray-500 py-20'>
              Please <span className='text-blue-600 font-medium'>sign in</span> to view portfolios.
            </div>
          ) : loading ? (
            // Skeleton loader
                        ...
                        <div className='flex items-center gap-2'>
                          <div className='relative w-8 h-8 rounded-full overflow-hidden bg-gray-200'>
                            {item.photographer_profiles?.profile_image_url ? (
                              <Image
                                src={item.photographer_profiles.profile_image_url}
                          ...
                          <span className='text-sm font-medium text-gray-800'>
                            {item.photographer_profiles?.full_name || 'Unknown Photographer'}
                          </span>
                        </div>
