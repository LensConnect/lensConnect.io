...
export default function PhotographerSetup() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setFormData((prev) => ({ ...prev, full_name: localUser.fullName || "", email: localUser.email || "" }))
    }
  }, [])
  ...
  const handleUpload = async () => {
    ...
    const { data, error } = await supabase.storage.from("profile_image").upload(fileName, imageFile)
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <p className="text-gray-500 text-[16px] mt-2">Let's set up your photographer getting bookings</p>
      </div>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input name="full_name" className=" mt-2" value={formData.full_name} onChange={handleChange} />
                      {errors.full_name && <p className="text-red-500 mt-2 text-sm">{errors.full_name}</p>}
                    </div>

                    <div>
                      <Label>Email</Label>
                      <Input name="email" className=" mt-2" value={formData.email} onChange={handleChange} />
                      {errors.email && <p className="text-red-500 mt-2 text-sm">{errors.email}</p>}
                    </div>

                    <div>
                      <Label>Phone</Label>
                      <Input name="phone" className=" mt-2" value={formData.phone} onChange={handleChange} />
                  ...
                  <div>
                    ...
                    <Input name="location" className=" mt-2" value={formData.location} onChange={handleChange} />
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input name="experience" className=" mt-2" value={formData.experience} onChange={handleChange} />
                      {errors.experience && <p className="text-red-500 mt-2 text-sm">{errors.experience}</p>}
                    </div>

                    <div>
                      <Label>Hourly Rate ($)</Label>
                      <Input name="hourly_rate" className=" mt-2" value={formData.hourly_rate} onChange={handleChange} />
                  ...
                  <div>
                    ...
                    <Input name="portfolio_url" className=" mt-2" value={formData.portfolio_url} onChange={handleChange} />
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setFormData((p) => ({ ...p, availability: checked }))}
                    />
                  </div>
                </>
              )}
